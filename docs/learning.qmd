...
For the participant details we know that the start date is before the end date so we don't care about keeping the column "name" when pivoting as we are just interested by the "value" that we can call "date".
